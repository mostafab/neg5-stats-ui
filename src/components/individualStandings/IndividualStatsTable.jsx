...
const HEADERS = router => [
    ...
    { displayName: 'Player Name', field: (player, tournamentId, slug, phaseId) => {
      ...
      return <a onClick={e => onClick(e, router, formattedUrl)} href={formattedUrl}> {player.playerName} </a>
    }},
...
const IndividualStatsTable = ({
  ...
  const getTableHeaders = () => {
    ...
    copy.find(h => h.displayName === 'Player Name').args = [tournamentId, slug, phaseId];
        ...
        <tbody>
          {
            individualStats.map(player =>
              <ObjectTableRow key={player.playerId} dataObject={player} headers={tableHeaders} />)
          }
        </tbody>
    </Table>
  );
