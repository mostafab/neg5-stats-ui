...
const TeamsAggregateStatsByBracketWrapper = ({
  ...
  const nonEmptyBrackets = brackets.filter(
    (bracket) => groupedTeams[bracket.id]
  );
    ...
    <div className="TeamsAggregateStatsByBracketWrapper">
      {nonEmptyBrackets.map((bracket) => (
        <TeamsAggregateStatsTable
          key={bracket.id}
          allTeamStats={groupedTeams[bracket.id]}
      ...
      {teamsWithoutBracket.length > 0 ? (
        <TeamsAggregateStatsTable
          key={null}
          allTeamStats={teamsWithoutBracket}
          tossupValues={tossupValues}
          bracket={UNASSIGNED_TEAMS_BRACKET}
          tournamentId={tournamentId}
          phaseId={phaseId}
          slug={slug}
          usesNegs={usesNegs}
