...
const TEAM_URL =
  "/t/{tournamentId}/{slug}/team-full?phase={phaseId}#team_{teamId}";
...
const HEADERS = (router) => [
  ...
  {
    displayName: "Team",
    field: (team, tournamentId, slug, phaseId) => {
      ...
      return (
        <a onClick={(e) => onClick(e, router, url)} href={url}>
          {" "}
          {team.teamName}{" "}
        </a>
      );
...
const TeamsAggregateStatsTable = ({
  ...
  const getTableHeaders = () => {
    ...
    copy.find((h) => h.displayName === "Team").args = [
      tournamentId,
      slug,
      phaseId,
    ];
  ...
  const bracketHeader = bracket ? (
    <tr>
      <th colSpan={tableHeaders.length}> {bracket.name} </th>
    </tr>
  ) : null;
  return (
