...
const mapStateToProps = (state) => ({
  allTeamStats: enrichStandings(
    state.teamStandings.allTeamStats,
    state.globalState.teams,
    state.globalState.selectedPhaseId,
    state.globalState.loadedTournament.currentPhaseId
  ),
  ...
  const orderedStats = orderBy(
    stats.map((teamStat) => {
      ...
      if (selectedPhaseId && team) {
        const matchingDivision = team.divisions.find(
          (div) => div.phaseId === selectedPhaseId
        );
        if (matchingDivision) {
        ...
        const matchingDivision = team.divisions.find(
          (div) => div.phaseId === currentPhaseId
        );
        if (matchingDivision) {
          divisionName = matchingDivision.name;
          divisionId = matchingDivision.id;
...
export default connect(mapStateToProps)(TeamStandingsRoot);
